...
# ---------------------------------
# UNIVERSAL FLASHSCORE FEED SCANNER
# ---------------------------------
async def get_live_matches():
    global ACTIVE_FEED
    FEED_FAMILIES = ["ls","l","lc","le","la","lr","lz","s","m","t"]
    SUFFIXES = ["1_0_0_en_1","1_0_1_en_1","1_0_2_en_1","1_0_3_en_1","1_0_4_en_1","1_0_5_en_1","1_0_6_en_1"]
    for family in FEED_FAMILIES:
        for suffix in SUFFIXES:
            feed = f"{family}_{suffix}"
            url = f"https://d.flashscore.com/x/feed/{feed}"
            logger.info(f"Trying UNIVERSAL feed: {feed}")
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.get(url, headers={"User-Agent": "Mozilla/5.0"}) as response:
                        ...
                        try:
                            ...
                            continue
                        match_ids = [item[1] for item in data if isinstance(item, list) and len(item)>1 and item[0]=="event"]
                        if match_ids:
                            ACTIVE_FEED = feed
                            logger.info(f"UNIVERSAL ACTIVE FEED FOUND: {feed} with {len(match_ids)} matches")
                            return match_ids
            except Exception as e:
                logger.error(f"Universal feed error ({feed}): {e}")
                continue
    ACTIVE_FEED = "None working"
    logger.warning("Universal scanner: No working feed found.")
    return []
...
def default_stats():
    return {"minute":0,"score":"0-0","shots_on_target":0,"dangerous_attacks":0,"home":"","away":""}
...
async def get_live_odds(match_id):
    ...
    odds = {"over05": None}
    for item in data:
        if isinstance(item, list) and len(item)>2 and item[0]=="odds" and item[1]=="O0.5":
            try:
...
async def check_matches(context: CallbackContext):
    ...
    for match_id in match_ids:
        try:
            ...
            continue
        match_name = f"{stats['home']} vs {stats['away']}".strip() or f"Match {match_id}"
